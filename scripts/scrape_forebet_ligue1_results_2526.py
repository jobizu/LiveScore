...
SOURCE_URL = "https://www.forebet.com/en/football-tips-and-predictions-for-france/ligue1/results"
DEFAULT_OUTPUT = Path("data/ligue1_forebet_results_2025_2026.csv")
...
def parse_rows(html: str) -> list[dict[str, str]]:
    ...
    if table is None:
        raise ValueError("Could not find Forebet Ligue 1 results table.")
    ...
    if not rows:
        raise ValueError("No Ligue 1 result rows were parsed from Forebet.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape Forebet Ligue 1 2025/26 results into a minimal CSV."
    )
...
def main() -> int:
    ...
    print(f"Saved Forebet Ligue 1 results CSV to: {args.output.resolve()}")
