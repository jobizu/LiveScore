...
SOURCE_URL = "https://www.fotmob.com/leagues/57/table/eredivisie"
DEFAULT_OUTPUT = Path("data/eredivisie_fotmob_table_2025_2026.csv")
OUTPUT_HEADERS = [
    "Position",
    "Team",
    "TeamId",
...
def parse_table_rows(next_data: dict) -> list[dict[str, str]]:
    ...
    if not standings:
        raise ValueError("No Eredivisie standings rows found in FotMob payload.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape FotMob Eredivisie table and team logo URLs into CSV."
    )
...
def main() -> int:
    ...
    print(f"Saved FotMob Eredivisie table CSV to: {args.output.resolve()}")
