...
SOURCE_URL = "https://www.fotmob.com/leagues/47/table/premier-league"
DEFAULT_OUTPUT = Path("data/epl_fotmob_table_2025_2026.csv")
OUTPUT_HEADERS = [
    "Position",
    "Team",
    "TeamId",
...
def parse_table_rows(next_data: dict) -> list[dict[str, str]]:
    ...
    if not standings:
        raise ValueError("No Premier League standings rows found in FotMob payload.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape FotMob Premier League table and team logo URLs into CSV."
    )
...
def main() -> int:
    ...
    print(f"Saved FotMob EPL table CSV to: {args.output.resolve()}")
