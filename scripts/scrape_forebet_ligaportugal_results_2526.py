...
SOURCE_URL = "https://www.forebet.com/en/football-tips-and-predictions-for-portugal/liga-portugal/results"
DEFAULT_OUTPUT = Path("data/ligaportugal_forebet_results_2025_2026.csv")
...
def parse_rows(html: str) -> list[dict[str, str]]:
    ...
    if table is None:
        raise ValueError("Could not find Forebet Liga Portugal results table.")
    ...
    if not rows:
        raise ValueError("No Liga Portugal result rows were parsed from Forebet.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape Forebet Liga Portugal 2025/26 results into a minimal CSV."
    )
...
def main() -> int:
    ...
    print(f"Saved Forebet Liga Portugal results CSV to: {args.output.resolve()}")
