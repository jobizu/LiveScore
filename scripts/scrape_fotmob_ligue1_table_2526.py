...
SOURCE_URL = "https://www.fotmob.com/leagues/53/table/ligue-1"
DEFAULT_OUTPUT = Path("data/ligue1_fotmob_table_2025_2026.csv")
OUTPUT_HEADERS = [
    "Position",
    "Team",
    "TeamId",
...
def parse_table_rows(next_data: dict) -> list[dict[str, str]]:
    ...
    if not standings:
        raise ValueError("No Ligue 1 standings rows found in FotMob payload.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape FotMob Ligue 1 table and team logo URLs into CSV."
    )
...
def main() -> int:
    ...
    print(f"Saved FotMob Ligue 1 table CSV to: {args.output.resolve()}")
