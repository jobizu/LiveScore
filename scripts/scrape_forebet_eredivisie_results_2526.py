...
SOURCE_URL = "https://www.forebet.com/en/football-tips-and-predictions-for-netherlands/eredivisie/results"
DEFAULT_OUTPUT = Path("data/eredivisie_forebet_results_2025_2026.csv")
...
def parse_rows(html: str) -> list[dict[str, str]]:
    ...
    if table is None:
        raise ValueError("Could not find Forebet Eredivisie results table.")
    ...
    if not rows:
        raise ValueError("No Eredivisie result rows were parsed from Forebet.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape Forebet Eredivisie 2025/26 results into a minimal CSV."
    )
...
def main() -> int:
    ...
    print(f"Saved Forebet Eredivisie results CSV to: {args.output.resolve()}")
