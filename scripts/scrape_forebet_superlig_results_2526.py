...
SOURCE_URL = "https://www.forebet.com/en/football-tips-and-predictions-for-turkey/super-lig/results"
DEFAULT_OUTPUT = Path("data/superlig_forebet_results_2025_2026.csv")
...
def parse_rows(html: str) -> list[dict[str, str]]:
    ...
    if table is None:
        raise ValueError("Could not find Forebet Super Lig results table.")
    ...
    if not rows:
        raise ValueError("No Super Lig result rows were parsed from Forebet.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape Forebet Turkiye Super Lig 2025/26 results into a minimal CSV."
    )
...
def main() -> int:
    ...
    print(f"Saved Forebet Super Lig results CSV to: {args.output.resolve()}")
